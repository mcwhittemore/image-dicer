...
module.exports = function(hooks) {
  var outlier = hooks.outlier || require('./outlier');
  var score = hooks.score || require('./score');
  ...
  Polygon.prototype.getScore = function() {
    if (this.score === null) {
      if (this.getOutlierData().length === 0) {
        this.score = -1;
      }
      else {
        this.score = score(this.data);
      }
    }
    return this.score;
  }
  ...
  Polygon.prototype.getOutlierData = function() {
    if (this.outlierData === null) {
      this.outlierData = this.data.filter(p => {
        var id = [p.x, p.y].join(',');
        return this.id.indexOf(id) === -1;
      });
    }
    return this.outlierData;
  };

  Polygon.prototype.getOutlier = function(points) {
    if (this.outlier === null) {
      this.outlier = outlier(this.getOutlierData(), this.getAvg());
    }
    return this.outlier;
  };
